...
func TestTokens(t *testing.T) {
	s, err := NewStoreWrapper()
	require.NoError(t, err)
	assert.NoError(t, s.store.SaveUserToken(Token{ID: "1", Owner: "test1", Scope: 7, Description: "test token", Token: "token1_1"}))
	assert.NoError(t, s.store.SaveUserToken(Token{ID: "2", Owner: "test1", Scope: 7, Description: "test token", Token: "token1_2"}))
	assert.NoError(t, s.store.SaveUserToken(Token{ID: "3", Owner: "test1", Scope: 7, Description: "test token", Token: "token1_3"}))
	assert.NoError(t, s.store.SaveUserToken(Token{ID: "4", Owner: "test2", Scope: 7, Description: "test token", Token: "token2_1"}))
